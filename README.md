...
Направление задавал я: разбирался в протоколе и проверял каждый шаг на реальной мыши. Код писался и переписывался вместе с ИИ-ассистентом (модель deepseek-v4-flash).
...
| `0x0060`–`0x009F` | кнопочная матрица и т. п.; область RGB-подсветки начинается на `0x00A0` (см. ниже), за ней память пустая (`0xFF`) |

Основная RGB-подсветка — это main-RGB-запись на `0x00A0` формата `effect/rgb/speed/brightness/ck`. Коды эффектов различаются между прошивками: на проверенном экземпляре `0x00` = перелив, `0x07` = строб, `0x08` = выкл. Команды `led` в утилите пока нет, но регион доступен через `probe`.
...
| `1` | ошибка выполнения — устройство не найдено, нет прав, пакеты отвергнуты, нет ответа от конфиг-памяти; сюда же — runtime-проверки `dpi`: слот `--slot` вне `0..5`, DPI вне 50–6400 или не кратный 50 |
| `2` | ошибка аргументов командной строки (argparse): неизвестная команда, нечисловой DPI, частота вне 125/500/1000 для `set` |
